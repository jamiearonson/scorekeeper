...
// Every call here is best-effort, but a swallowed error is invisible — a missing table or
// RLS rejection looks identical to "nothing to save". Log it so failures are diagnosable
// (e.g. in the field) without changing the no-op-on-failure behavior callers rely on.
function logError(op: string, error: unknown): void {
  if (error) console.error(`[persistence] ${op} failed:`, error);
}
...
export async function listGamesInOccasion(groupId: string): Promise<Game[]> {
  ...
  const { data, error } = await supabase
    ...
    .eq("group_id", groupId)
    .order("updated_at", { ascending: false });
  logError("listGamesInOccasion", error);
  return (data as GameRow[] | null)?.map(rowToGame) ?? [];
}
